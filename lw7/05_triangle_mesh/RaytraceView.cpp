...
void CRaytraceView::AddSomeCube()
{
	...
	transform.Rotate(140, 0, 1, 0);
	CSimpleMaterial red;
	red.SetDiffuseColor(CVector4f(1.0f, 0, 0, 1));

	AddCube(CreateSimpleDiffuseShader(red), transform);
}
...
void CRaytraceView::AddSomeDodecahedron()
{
	...
	CSimpleMaterial yellow;
	yellow.SetDiffuseColor(CVector4f(0.8f, 1, 0, 1));

	AddDodecahedron(CreateSimpleDiffuseShader(yellow), transform);
}
...
void CRaytraceView::AddSomeIcosahedron()
{
	...
	transform.Rotate(0, 1, 0, 0);
	CSimpleMaterial purple;
	purple.SetDiffuseColor(CVector4f(0.9f, 0, 0.8f, 1));

	AddIcosahedron(CreateSimpleDiffuseShader(purple), transform);
}
...
void CRaytraceView::AddSomeOctahedron()
{
	...
	CMatrix4d transform;
	transform.Translate(2, 1.5, 4);
	transform.Rotate(90, 0, 1, 0);
	transform.Rotate(0, 1, 0, 0);
	CSimpleMaterial green;
	green.SetDiffuseColor(CVector4f(0, 1, 0, 1));

	AddOctahedron(CreateSimpleDiffuseShader(green), transform);
...
CSceneObject& CRaytraceView::AddTriangleMesh(IShader const& shader, std::shared_ptr<CTriangleMeshData const> pMeshData, CMatrix4d const& transform)
{
...
CSceneObject& CRaytraceView::AddDodecahedron(IShader const& shader, CMatrix4d const& transform)
{
	const auto& mesh = *m_geometryObjects.emplace_back(std::make_unique<CDodecahedron>(transform));
	return AddSceneObject(mesh, shader);
}
...
{
	const auto& mesh = *m_geometryObjects.emplace_back(std::make_unique<CCube>(transform));
	return AddSceneObject(mesh, shader);
}

CSceneObject& CRaytraceView::AddTetrahedron(IShader const& shader, CMatrix4d const& transform)
{
	const auto& mesh = *m_geometryObjects.emplace_back(std::make_unique<CTetrahedron>(transform));
	return AddSceneObject(mesh, shader);
}

CSceneObject& CRaytraceView::AddIcosahedron(IShader const& shader, CMatrix4d const& transform)
{
	const auto& mesh = *m_geometryObjects.emplace_back(std::make_unique<CIcosahedron>(transform));
	return AddSceneObject(mesh, shader);
}

CSceneObject& CRaytraceView::AddOctahedron(IShader const& shader, CMatrix4d const& transform)
{
	const auto& mesh = *m_geometryObjects.emplace_back(std::make_unique<COctahedron>(transform));
	return AddSceneObject(mesh, shader);
}
